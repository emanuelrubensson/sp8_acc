...
def plot_convergence_sp5(X, D, Z, L_out, L_in, H_in, H_out, ax_cond, ax_idem, marker, color, linestyle, label=''):
    X0 = np.copy(X)
    sp5_out = sp5.sp5_acc(X0, L_out, L_in, H_in, H_out, expensive_output=1)
    # sp5_out: X, nmul, polys, gap, nmul_vec, idem_err_trace, idem_err_maxabs
    #          0  1     2      3    4         5               6
    check_result(sp5_out[0],D,Z)
    gap = sp5_out[3]
    nmul_vec = sp5_out[4]
    idem_err_maxabs = sp5_out[6]
    X0 = np.copy(X)
    sp5_out_extraiter = sp5.sp5_acc(X0, L_out, L_in, H_in, H_out, expensive_output=1,fixed_niter=len(nmul_vec)+2)
    nmul_vec_poststop = sp5_out_extraiter[4][len(nmul_vec)-1:]
    idem_err_maxabs_poststop = sp5_out_extraiter[6][len(nmul_vec)-1:]
    ax_cond.semilogy(nmul_vec,[1/x for x in gap[0:len(nmul_vec)]],
                     color=color, marker=marker, linestyle=linestyle,
                     linewidth=1, markersize=3, label=label)
    ax_idem.semilogy(nmul_vec_poststop[0:2],idem_err_maxabs_poststop[0:2],
...
print(f"Gap of X0: H_outer - L_outer : {H_outer - L_outer}")
print(f"Gap of X0: H_inner - L_inner : {H_inner - L_inner}")
...
plot_convergence_sp5(X0, D, Z, L_outer, L_inner, H_inner, H_outer, ax1, ax2, 's', 'tab:red', 'solid', 'Su5')
